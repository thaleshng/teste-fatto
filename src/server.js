...
app.post("/tarefas", async (req, res) => {
    ...
    if (!nome || custo === undefined || !data_limite) {
        return res.status(400).json({
            message: "Todos os campos (nome, custo, data_limite) são obrigatórios.",
        });
    }
...
app.put("/tarefas/reordenar", async (req, res) => {
    ...
    try {
        await prisma.$transaction(tarefas.map((tarefa) => prisma.Tarefas.update({
            where: { id: tarefa.id },
            data: { ordem_apresentacao: tarefa.id * 1000 },
        })));
        await prisma.$transaction(tarefas.map((tarefa, index) => prisma.Tarefas.update({
            where: { id: tarefa.id },
